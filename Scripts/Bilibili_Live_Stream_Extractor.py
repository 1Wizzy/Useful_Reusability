...
class BilibiliLiveStreamExtractor:
                        ...
                        streams.setdefault(stream_key, []).append(
                            {
                                ...
                                "quality_name": self.quality_map.get(
                                    current_qn, f"未知({current_qn})"
                                ),
                            }
                        )
        return streams
    ...
    def print_stream_info(self, all_streams):
        ...
        print("B站直播流链接信息")
        print("=" * 80)
        for qn, stream_info in sorted(
            all_streams.items(), key=lambda x: x[0], reverse=True
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument("live_url", help="Live stream url")
    parser.add_argument("-all", "--allstream", help="print all streams", action="store_true")
